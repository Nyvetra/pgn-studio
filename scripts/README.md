...
| `build-pgn-extract.sh` | Written, **UNVERIFIED** (macOS/Apple clang) | Mirrors the Windows script's contract for `aarch64-apple-darwin`/`x86_64-apple-darwin` (no TRE - system libc regex). Never executed - no Mac is available in this development environment (decisions ledger D-006). Treat its first real run as verification, not a formality. |
